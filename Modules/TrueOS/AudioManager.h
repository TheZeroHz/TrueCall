#ifndef AUDIOMANAGER_H  // âœ… Fixed from #ifdef
...
enum : uint8_t { SET_VOLUME, GET_VOLUME, CONNECTTOHOST, CONNECTTOSD, CONNECTTOSPEECH, GET_AUDIOSTATUS };
  ...
  while (true) {
    if (xQueueReceive(audioSetQueue, &audioRxTaskMessage, 1) == pdPASS) {
      if (audioRxTaskMessage.cmd == SET_VOLUME) {
        ...
        audioTxTaskMessage.ret = audio.connecttoFS(FFat,audioRxTaskMessage.txt);
        xQueueSend(audioGetQueue, &audioTxTaskMessage, portMAX_DELAY);
...
void audioInit() {
  xTaskCreatePinnedToCore(audioTask, "audioplay", 5000, NULL, 2 | portPRIVILEGE_BIT, NULL, 1);
}
...
bool audioConnecttoFFat(const char* filename) {
