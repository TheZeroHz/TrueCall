    ...
    toItem(0)
{
  ...
  // Create the ONLY full-screen sprite (canvas)
  spr.setColorDepth(16);
  spr.createSprite(SCREEN_WIDTH, SCREEN_HEIGHT);
  spr.setSwapBytes(true); // most RGB565 sources need this byte order
}
...
void TFT_GUI::displayHardwareInfo(bool sdMounted, uint64_t sdTotalSize, uint64_t sdUsedSpace, const char* sdType) {
  // Background based on FFat status
...
bool TFT_GUI::jpegRenderToSprite(TFT_eSprite* dst, int xpos, int ypos) {
  ...
  // Ensure sprite is RGB565 with proper byte order
...
void TFT_GUI::renderUI() {
  ...
  // Optionally overlay another JPEG:
  // drawJpegToSprite(&person, "/avatar.jpg", 0, 0);
  person.pushToSprite(&spr, SCREEN_WIDTH - 96 - 8, 8, TFT_BLACK);

  // Push final composed frame to TFT
  spr.pushSprite(0, 0);
}
